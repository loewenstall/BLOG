 ...
 * 
 * A merge of the StandaloneView and TemplateView, modified to
 * work completly wihout TYPO3 Flow
 *
 * @api
 */
class StandaloneView extends \TYPO3\Fluid\View\AbstractTemplateView {
    
    /**
     * @var \TYPO3\Fluid\View\ViewContext
     */
	 ...
	 * @var string
	 */
	protected $templateRootPathPattern = '@fluidResourcesPath/Templates';
	 ...
	 * @var string
	 */
	protected $partialRootPathPattern = '@fluidResourcesPath/Partials';
	 ...
	 * @var string
	 */
	protected $layoutRootPathPattern = '@fluidResourcesPath/Layouts';
	...
	public function setFormat($format) {
		$this->viewContext()->setFormat($format);
	}
	...
	public function getFormat() {
		return $this->viewContext->getFormat();
	}
	...
	protected function getTemplateIdentifier($actionName = NULL) {
		...
		if ($actionName === NULL) {
			$actionName = $this->templateContext->getControllerActionName();
		}
	...
	protected function getTemplatePathAndFilename($actionName = NULL) {
        ...
        if (\TYPO3\Fluid\Fluid::$resourcesPath === NULL) {
            throw new \TYPO3\Fluid\View\Exception\InvalidTemplateResourceException('Fluid Standalone resources path needs to be defined for dynamic template resolving.', 1366142697);
        }
		if ($actionName === NULL) {
			$actionName = $this->templateContext->getControllerActionName();
		}
	...
	protected function getLayoutPathAndFilename($layoutName = 'Default') {
        ...
        if (\TYPO3\Fluid\Fluid::$resourcesPath === NULL) {
            throw new \TYPO3\Fluid\View\Exception\InvalidTemplateResourceException('Fluid Standalone resources path needs to be defined for dynamic layout resolving.', 1366142697);
        }
	...
	protected function getPartialSource($partialName) {
		$partialPathAndFilename = $this->getPartialPathAndFilename($partialName);
		$partialSource = \TYPO3\Fluid\Utility\Files::getFileContents($partialPathAndFilename, FILE_TEXT);
	...
	public function getTemplateRootPath() {
		if ($this->templateRootPath !== NULL) {
			...
			return str_replace('@fluidResourcesPath', \TYPO3\Fluid\Fluid::$resourcesPath, $this->templateRootPathPattern);
		}
	}
	...
	protected function getPartialRootPath() {
		if ($this->partialRootPath !== NULL) {
			...
			return str_replace('@fluidResourcesPath', \TYPO3\Fluid\Fluid::$resourcesPath, $this->partialRootPathPattern);
		}
	}
	...
	protected function getLayoutRootPath() {
		if ($this->layoutRootPath !== NULL) {
			...
			return str_replace('@fluidResourcesPath', \TYPO3\Fluid\Fluid::$resourcesPath, $this->layoutRootPathPattern);
		}
	}
	 ...
	 * Standalone_<ControllerName>_<prefix>_<SHA1>
	...
	protected function createIdentifierForFile($pathAndFilename, $prefix) {
		...
		$templateIdentifier = sprintf('Standalone_%s_%s_%s', $controllerName, $prefix, sha1($pathAndFilename . '|' . $templateModifiedTimestamp));
    ...
    protected function buildParserConfiguration() {
    	...
    	if (in_array($this->templateContext->getFormat(), array('html', NULL))) {
			$parserConfiguration->addInterceptor(new \TYPO3\Fluid\Core\Parser\Interceptor\Escape());
			//$parserConfiguration->addInterceptor(new \TYPO3\Fluid\Core\Parser\Interceptor\Resource());
		}
