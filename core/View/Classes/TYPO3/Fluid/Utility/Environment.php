...
class Environment {
	...
	protected function createTemporaryDirectory() {
		if (!is_dir(TEMP_PATH) && !is_link(TEMP_PATH)) {
			try {
				\TYPO3\Fluid\Utility\Files::createDirectoryRecursively(TEMP_PATH);
			} catch (\TYPO3\Flow\Error\Exception $exception) {
				throw new \TYPO3\Flow\Utility\Exception('The temporary directory "' . TEMP_PATH . '" could not be created. Please make sure permissions are correct for this path or define another temporary directory in your Settings.yaml with the path "TYPO3.Flow.utility.environment.temporaryDirectoryBase".', 1335382361);
			}
		}

		if (!is_writable(TEMP_PATH)) {
			throw new \TYPO3\Fluid\Utility\Exception('The temporary directory "' . TEMP_PATH . '" is not writable. Please make this directory writable or define another temporary directory in your Settings.yaml with the path "TYPO3.Flow.utility.environment.temporaryDirectoryBase".', 1216287176);
		}
